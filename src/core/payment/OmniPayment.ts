...
export class OmniPayment {
  /**
   * The omniPay method is used to make a payment with token of your choice.
  ...
  async omniPay({
    ...
    const toChainId = requiredToken.chainId;
    const fromChainId = paidToken.chainId;
    ...
    if (IS_EVM(paidToken.chainId) && options.signerOrProvider == undefined) {
    ...
    if (
      ChainId.NEAR_TESTNET == paidToken.chainId &&
    ...
    if (IS_EVM(fromChainId)) {
      swapData = await assembleTargetSwapDataFromRoute(route, requiredToken);
    }
